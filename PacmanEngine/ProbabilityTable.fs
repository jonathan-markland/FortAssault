...
let MovementProbabilitiesFor railsBitmask facingDirection =
    ...
    else if railsBitmask = MazeByteLeft then
        { ProbLeft=1uy ; ProbUp=0uy  ; ProbRight=0uy  ; ProbDown=0uy  }

    else if railsBitmask = MazeByteUp then
        { ProbLeft=0uy ; ProbUp=1uy  ; ProbRight=0uy  ; ProbDown=0uy  }

    else if railsBitmask = MazeByteRight then
        { ProbLeft=0uy ; ProbUp=0uy  ; ProbRight=1uy  ; ProbDown=0uy  }

    else if railsBitmask = MazeByteDown then
        { ProbLeft=0uy ; ProbUp=0uy  ; ProbRight=0uy  ; ProbDown=1uy  }
